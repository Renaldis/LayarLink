...
export default async function PublicVideoPage({ params }: { params: Promise<{ publicId: string }> }) {
  ...
  if (!video) notFound();
  const playbackUrl = await createPlaybackUrl(video.objectKey);
  ...
  return (
    <main className="min-h-screen bg-[#fbfbfd] px-5 py-8 text-[#1d1a27] sm:px-10">
      <section className="mx-auto max-w-5xl">
        <Link className="inline-flex items-center gap-2 font-sans text-lg font-bold" href="/"><span className="grid size-8 place-items-center rounded-[10px] bg-[#625df5] text-white"><Video size={16} /></span>LayarLink</Link>
