...
import { EditableTitle } from "@/components/videos/editable-title";
import { FolderSidebar } from "@/components/videos/folder-sidebar";

function formatBytes(bytes: number) {
  return `${(bytes / 1024 / 1024).toFixed(1)} MB`;
}

export default async function VideosPage({ searchParams }: { searchParams: Promise<{ folder?: string }> }) {
  const session = await getCurrentSession();
  if (!session?.user) redirect("/signin");
  const { folder } = await searchParams;
  const [videos, folders] = await Promise.all([db.video.findMany({ where: { ownerId: session.user.id, deletedAt: null, ...(folder ? { folderId: folder } : {}) }, orderBy: { createdAt: "desc" } }), db.folder.findMany({ where: { ownerId: session.user.id }, include: { _count: { select: { videos: true } } }, orderBy: { name: "asc" } })]);
